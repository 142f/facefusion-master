...
from facefusion.filesystem import create_directory, get_file_extension, resolve_relative_path
from facefusion.temp_helper import get_temp_root_path
...
def pre_check() -> bool:
	...
	create_directory(examples_path)
	conditional_download(examples_path,
	[
...
def run() -> Iterator[List[BenchmarkCycleSet]]:
	...
	state_manager.init_item('source_paths', [ os.path.join(examples_path, 'source.jpg'), os.path.join(examples_path, 'source.mp3') ])
	...
	benchmarks = []
	target_paths = [ resolve_benchmark_path(facefusion.choices.benchmark_set[benchmark_resolution]) for benchmark_resolution in benchmark_resolutions if benchmark_resolution in facefusion.choices.benchmark_set ]
...
def suggest_output_path(target_path : str) -> str:
	...
	benchmark_path = os.path.join(get_temp_root_path(), 'benchmark')
	create_directory(benchmark_path)
	return os.path.join(benchmark_path, hashlib.sha1(target_path.encode()).hexdigest() + target_file_extension)


def resolve_examples_path() -> str:
	return resolve_relative_path('../.assets/examples')


def resolve_benchmark_path(target_path : str) -> str:
	return os.path.join(resolve_examples_path(), os.path.basename(target_path))
