...
def move_file(file_path : str, move_path : str) -> bool:
	if is_file(file_path):
		try:
			...
			# On Windows, shutil.move can finish copying but fail removing source.
			if is_file(move_path):
				try:
					os.remove(file_path)
				except OSError:
					pass
			...
			return False
		return (not is_file(file_path) and is_file(move_path)) or is_file(move_path)
	return False
...
def resolve_file_paths(directory_path : str) -> List[str]:
	...
	if directory_path and not os.path.isabs(directory_path) and not is_directory(directory_path):
		relative_directory_path = resolve_relative_path('../' + directory_path)

		if is_directory(relative_directory_path):
